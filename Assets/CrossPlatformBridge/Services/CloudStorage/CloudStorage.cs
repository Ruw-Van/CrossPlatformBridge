...
namespace CrossPlatformBridge.Services.CloudStorage
{
	...
	public class CloudStorage : MonoBehaviour
	{
		...
		/// </summary>
		/// <typeparam name="T"><see cref="ICloudStoragePlatform"/> を実装し、パラメーターなしコンストラクターを持つプラットフォーム型。</typeparam>
		/// <returns>生成されたハンドラー。</returns>
		public IInternalCloudStorageHandler Use<T>() where T : ICloudStoragePlatform, new()
		{
			var handler = new T().CreateCloudStorageHandler();
