...
namespace CrossPlatformBridge.Services.Network
{
	...
	public interface IInternalNetworkHandler
	{
		...
		/// </summary>
		/// <param name="conditions">検索条件（RoomName・MaxPlayers・CustomProperties）</param>
		/// <param name="cancellationToken">キャンセルトークン（省略可）</param>
		/// <returns>接続に成功した場合は true、失敗またはキャンセル時は false</returns>
		UniTask<bool> MatchmakeLobby(IRoomSettings conditions, CancellationToken cancellationToken = default);
		...
		UniTask<bool> MatchmakeRoom(IRoomSettings conditions, bool createIfNotFound = false, CancellationToken cancellationToken = default);
