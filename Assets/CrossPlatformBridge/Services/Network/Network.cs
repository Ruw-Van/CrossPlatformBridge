...
namespace CrossPlatformBridge.Services.Network
{
	...
	public partial class Network : MonoBehaviour, IInternalNetworkHandler // ★ クラス名変更
	{
		...
		/// <summary>
		/// 指定したプラットフォームのハンドラーを生成して初期化します。
		/// </summary>
		/// <typeparam name="T"><see cref="INetworkPlatform"/> を実装し、パラメーターなしコンストラクターを持つプラットフォーム型。</typeparam>
		/// <returns>初期化に成功した場合は true、失敗した場合は false。</returns>
		public UniTask<bool> Use<T>() where T : INetworkPlatform, new()
		{
#pragma warning disable CS0618
			return InitializeLibrary(new T().CreateNetworkHandler());
#pragma warning restore CS0618
		}

		[System.Obsolete("Use UseLibrary<T>() instead for parameterless handlers. " +
		                 "InitializeLibrary remains available when constructor arguments are required.")]
		...
		public UniTask ShutdownLibrary()
		{
			...
			try
			{
				...
				// MonoBehaviour ベースのハンドラ（PUN2 等）は GameObject ごと破棄する
				if (_internalNetworkHandler is MonoBehaviour mb)
