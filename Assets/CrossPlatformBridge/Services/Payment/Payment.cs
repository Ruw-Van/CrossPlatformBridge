...
namespace CrossPlatformBridge.Services.Payment
{
	...
	public class Payment : MonoBehaviour
	{
		...
		/// </summary>
		/// <typeparam name="T"><see cref="IPaymentPlatform"/> を実装し、パラメーターなしコンストラクターを持つプラットフォーム型。</typeparam>
		/// <returns>生成されたハンドラー。</returns>
		public IInternalPaymentHandler Use<T>() where T : IPaymentPlatform, new()
		{
			var handler = new T().CreatePaymentHandler();
