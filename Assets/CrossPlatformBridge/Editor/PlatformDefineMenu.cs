...
namespace CrossPlatformBridge.Editor
{
    internal sealed class PlatformDefineWindow : EditorWindow
    {
        ...
        private static List<string> ScanPlatforms()
        {
            ...
            // Scan Assets/CrossPlatformBridge/Platform/ (local / submodule)
            ...
            // Also discover UPM platform packages (com.github.ruw-van.crossplatformbridge.<platform>)
            const string upmPrefix = "com.github.ruw-van.crossplatformbridge.";
            foreach (var info in UnityEditor.PackageManager.PackageInfo.GetAllRegisteredPackages())
            {
                if (!info.name.StartsWith(upmPrefix, StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }

                var suffix = info.name.Substring(upmPrefix.Length);
                if (suffix.Length == 0)
                {
                    continue;
                }

                var platformName = char.ToUpperInvariant(suffix[0]) + suffix.Substring(1);
                if (!names.Any(n => string.Equals(n, platformName, StringComparison.OrdinalIgnoreCase)))
                {
