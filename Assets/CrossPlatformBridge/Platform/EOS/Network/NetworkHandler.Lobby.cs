...
namespace CrossPlatformBridge.Platform.EOS.Network
{
	...
	public partial class NetworkHandler
	{
		...
		public async UniTask<List<object>> SearchLobby(IRoomSettings baseSettings)
		{
			...
			// CustomProperties でサーバー側フィルタ
			if (settings?.CustomProperties != null)
			{
				foreach (var kv in settings.CustomProperties)
				{
					var param = new LobbySearchSetParameterOptions
					{
						ComparisonOp = ComparisonOp.Equal,
						...
						{
							Key = kv.Key,
							Value = new AttributeDataValue { AsUtf8 = kv.Value?.ToString() ?? "" },
						},
					};
					search.SetParameter(ref param);
				}
				...
				if (result.ResultCode == Result.Success)
				{
					...
					for (uint i = 0; i < count; i++)
					{
						...
						if (details != null)
						{
							...
							details.CopyInfo(ref copyInfoOptions, out var lobbyInfo);
							if (lobbyInfo != null)
							{
								list.Add(new RoomSettings
								{
									Id = lobbyInfo.Value.LobbyId,
									LobbyId = lobbyInfo.Value.LobbyId,
									RoomName = lobbyInfo.Value.BucketId ?? "",
