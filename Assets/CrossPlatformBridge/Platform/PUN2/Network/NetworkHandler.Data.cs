...
namespace CrossPlatformBridge.Platform.PUN2.Network
{
	...
	/// </summary>
	public partial class NetworkHandler : MonoBehaviourPunCallbacks, IInternalNetworkHandler, IOnEventCallback
	{
		// --------------------------------------------------------------------------------
		// 内部定数
		// --------------------------------------------------------------------------------
		...
		public void OnEvent(ExitGames.Client.Photon.EventData photonEvent) // ExitGames.Client.Photon.EventData を明示
		{
			if (photonEvent.Code == DATA_EVENT_CODE)
			{
				...
				if (data != null)
				{
					...
					// photonEvent.Sender は ActorNumber (int)。UserId は CurrentRoom から取得する
					var senderPlayer = PhotonNetwork.CurrentRoom?.GetPlayer(photonEvent.Sender);
					OnDataReceived?.Invoke(data, senderPlayer?.UserId ?? photonEvent.Sender.ToString()); // 受信イベントを発火
				}
			}
		}
	}
