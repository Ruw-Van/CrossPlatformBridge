...
namespace CrossPlatformBridge.Platform.PUN2.Network
{
	...
	public partial class NetworkHandler : MonoBehaviourPunCallbacks, IInternalNetworkHandler, IServiceTestProvider
	{
		...
		/// <summary>
		/// Connect() 実行前に呼び出される認証フック。
		/// プラットフォーム固有の AuthValues 設定などに使用します。
		/// RuntimeInitializeOnLoadMethod から登録してください。
		/// </summary>
		public static Func<UniTask> PreConnectAuthAsync { get; set; }
		...
		public async UniTask<bool> Connect(INetworkSettings baseSettings)
		{
			...
			// 接続前認証フック（Switch の NintendoSwitch 認証、Steam 認証など）
			if (PreConnectAuthAsync != null)
				await PreConnectAuthAsync();
