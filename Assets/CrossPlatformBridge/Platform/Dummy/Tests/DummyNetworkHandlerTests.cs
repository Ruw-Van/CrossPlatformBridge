...
namespace CrossPlatformBridge.Platform.Dummy.Tests
{
    ...
    public class DummyNetworkHandlerTests
    {
        ...
        public IEnumerator MatchmakeLobby_NameMatch_ConnectsSuccessfully() => UniTask.ToCoroutine(async () =>
        {
            ...
            Assert.IsTrue(result, "名前がマッチするロビーへの MatchmakeLobby は true を返す必要があります");
            Assert.IsNotNull(_handler.StationId, "MatchmakeLobby 後は StationId が設定される必要があります");
        });
        ...
        public IEnumerator MatchmakeLobby_NoMatch_ReturnsFalse() => UniTask.ToCoroutine(async () =>
        {
            ...
            Assert.IsFalse(result, "マッチするロビーがない場合は false を返す必要があります");
        });
        ...
        public IEnumerator MatchmakeRoom_NameMatch_ConnectsSuccessfully() => UniTask.ToCoroutine(async () =>
        {
            ...
            Assert.IsTrue(result, "名前がマッチするルームへの MatchmakeRoom は true を返す必要があります");
            Assert.IsNotNull(_handler.StationId);
        });
        ...
        public IEnumerator MatchmakeRoom_NoMatch_CreateIfNotFound_CreatesRoom() => UniTask.ToCoroutine(async () =>
        {
            ...
            Assert.IsTrue(result, "createIfNotFound=true の場合、新規作成して true を返す必要があります");
            Assert.IsTrue(_handler.IsHost, "新規作成時は IsHost が true になる必要があります");
        });
        ...
        public IEnumerator MatchmakeRoom_CustomPropertiesFilter_OnlyMatchingRoom() => UniTask.ToCoroutine(async () =>
        {
            ...
            Assert.IsTrue(result, "CustomProperties がマッチするルームへの接続は true を返す必要があります");
        });
    }
}
